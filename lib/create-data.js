...
const seededMmrPatients = [
  ...
  },
  {
    ...
    doses: [
      // Case 2 — one valid Dose 1, then a long trail of failed attempts to
      // give Dose 2. Bilal still needs a second MMR dose.
      {
  ...
  },
  {
    ...
    doses: [
      ...
      // one Dose 1, but recorded three times: the originating GP record plus
      // two echoes from other feeds. Eshe still needs Dose 2; the duplicates
      // matter now because they obscure the schedule picture.
      ...
      // Dose 1 second echo — another feed, one day after the first echo
      {
...
// Bilal's Y9 Doubles session last academic year (AY 2024/25, summer term).
// He was off school sick on the day, so both the MenACWY and Td/IPV doses
// were recorded as absent. Sets up the demo where the team is now seeing
// Bilal in Y10 and can see he still needs the teenage boosters too.
...
for (const entry of bilalDoublesAbsences) {
  ...
  const vaccination = new Vaccination(
    {
      ...
      note: 'Off school with viral illness on the day of the team visit.'
    },
    context
  )
  context.vaccinations[vaccination.uuid] = vaccination
